...
for game in 10 ; do #{11,15,3,4,5,6,7,8,9}; do # GAMES HERE

	date_start_string=`ls ./data/round$round/rando_g$game/ | grep checkpoint | sed 's/_/ /g' | awk '{print \$3}' | uniq`
	...
	for strategy in {hand_max_min,hand_max_avg,hand_max_med,hand_max_poss,crib_min_avg,pegging_max_avg_gained,pegging_max_med_gained,pegging_min_avg_given}; do
		...
		for agent in agent1 ; do #{agent1,agent2}; do # AGENT NAMES HERE

			echo "Generating images for $agent : $strategy during $game"

			outdir_base="images/training/round$round/rando_g$game"
			...
			./analyze.py $operation\
				...
				--directory "data/round$round/rando_g$game"\
